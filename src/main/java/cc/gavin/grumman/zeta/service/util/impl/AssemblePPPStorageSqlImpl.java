 ...
 */
public class AssemblePPPStorageSqlImpl implements AssembleSql{

    private XSSFSheet xssfSheet;

    public AssemblePPPStorageSqlImpl(XSSFSheet xssfSheet){
    ...
    public List<String> getSqlList() {
        ...
        Map<String,Integer> map  =  ExcelUtil.getIndexMap(new String[]{"单据日期","单据类型","单据编码","物料类别","物料及规格","单位","收料数量","发料数量","条只","单价","收入金额","发出金额","收料部门","发料部门","供应商"},title_xssfRow);
        ...
        for (int rowNum = 1; rowNum <= xssfSheet.getLastRowNum(); rowNum++) {
            ...
            if (xssfRow != null) {
                String sql = "insert into `ppp_storage_info` ( `docket_time`, `docket_type`,  `docket_code`, " +
                        ...
                        ",'"+ExcelUtil.getValue(xssfRow.getCell(map.get("条只")))+"','"+ExcelUtil.getValue(xssfRow.getCell(map.get("单价")))+"','"+ExcelUtil.getInt(ExcelUtil.getValue(xssfRow.getCell(map.get("收入金额"))))+"'" +
